...
import { fetch_dashboard_expense__service, fetch_dashboard_payment__service, fetch_dashboard_users__service } from "../services/dashboard-service";
...
export const fetchDashboard = async (req: Request, res: Response): Promise<void> => {
    ...
    const expenses: any = await fetch_dashboard_expense__service(schoolId)
    ...
    res.status(200).json({
        ...
        expense: {
            today: expenses[0][0].today == null ? 0 : expenses[0][0].today,
            month: expenses[1][0].month == null ? 0 : expenses[1][0].month
        }
    })
}